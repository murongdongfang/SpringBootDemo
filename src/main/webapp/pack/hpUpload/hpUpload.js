/**
 *     @隐无为 hpUpload base to AJAX Upload
...
(function($){
  ...
  var uploadOption_plus = {
    ...
    // 是否开启批量上传  默认是不开启
    multiple : true,
    // 是否是预览编辑初始化数据 默认是false
    isEditInitData:false,
    // 参数
    data : {
      "path" : "upload/headImg"
    },
    //选择添加按钮
    uploadSelect:"#kdViewUploadSelect",
    //开始上传按钮
    previewUploadBtn:"#kdUploadBtn",
    //预览区域div 
    previewUploadDiv:"#kdViewDiv",
    //删除的class
    ...
    onPreview:function(urls,selectorDiv,previewUploadBtn){
      // 判断有没有选择添加按钮 有就不填加了
      if(!$(this._settings.uploadSelect).length){
    	  var uploadSelect=this._settings.uploadSelect.slice(1);
        $(selectorDiv).append('<div  id="'+uploadSelect+'" class="hp-upload-div-kdUploadAdd"  style="float:left;"></div>');
      }
      // 填充预览图片
      var $htlml="";
      ...
      $(this._settings.uploadSelect).before($htlml);
      // 如果div是隐藏的 就显示出来
      $(selectorDiv).show();   
    },
    // 选择文件之后…
  ...
  $.extend({
    ...
    hpUpload:function(url,data,success,multiple,selector){
     ...
     // uploadOption.onSuccess=success;
      var uploadOpt=uploadOption;
      uploadOpt.onSuccess=success;
      if(multiple){
      ...
      // 初始化上传组件AjaxUpload
      var au = new AjaxUpload(select, uploadOpt);
      ajaxUpload=au;
      //添加对象到元素里
      $(select).data("ajaxUpload",au);
    },
    // plus-扩展  
    hpUpload_plus:{
      // 预览上传 带美化样式
      viewUpload:function(url,data,success,error){
            ...
            uploadOption_plus.data=data;
            uploadOption_plus.onSuccess=success; 
            uploadOption_plus.onError=error; 
            var uploadOpt=uploadOption_plus;
            // 添加初始样式
            var id=uploadOpt.uploadSelect.slice(1);
            var selectorDiv='<div id="'+id+'" class="hp-upload-div-img"></div>';
          $(uploadOption_plus.previewUploadDiv).append(selectorDiv);
          // 初始化上传组件AjaxUpload
          var au = new AjaxUpload(uploadOpt.uploadSelect, uploadOpt);
          ajaxUpload_plus=au; 
          //添加对象到元素里
          $(uploadOpt.previewUploadDiv).data("ajaxUpload",au);
      },
      // 预览上传 带美化样式 带上传数量限制和数据初始化
      viewUpload_num_edit:function(url,data,initData,success,error){
            uploadOption_plus.action=url;
            uploadOption_plus.isEditInitData=true;
            uploadOption_plus.data=data;
            uploadOption_plus.onSuccess=success; 
            uploadOption_plus.onError=error; 
            var uploadOpt=uploadOption_plus;
          ...
          // 初始化数据
        	  ...
        	  if(initData.length>0){
        	      ...
        	      $.each(initData,function(index,item){ 
        	    	  var guid =kd_guid();
        	    	  $htlml+='<div idx="'+guid+'" class="kdfile kdpvImg" style="float:left;width:110px;height:110px"><img  style="width:110px;height:110px" class="hp-upload-div-removeUploadBtnDiv-img" src="'+item+'" /><div class="hp-upload-div-removeUploadBtnDiv-btn"><span class="remove">删除</span></div></div>';
        	    	  $intData+='<p id="kd-hidden-img-'+guid+'">'+item+'</p>'
        	      }) 
        	      $('#kdViewUploadSelect').before($htlml);	
        	      
        		  $(uploadOption_plus.previewUploadDiv).append('<div id="kd-view-hidden" style="disPlay:none"></div>');	
        		  $('#kd-view-hidden').append($intData);	          		  
        	  }
      ...
      // 预览上传 带美化样式 自定义区域
      viewUploadArea:function(previewUploadDiv,url,data,success,error){
            uploadOption_plus.action=url;
            uploadOption_plus.data=data;
            uploadOption_plus.onSuccess=success; 
            uploadOption_plus.onError=error; 
            uploadOption_plus.previewUploadDiv=previewUploadDiv;
            uploadOption_plus.uploadSelect=previewUploadDiv+"Btn";
            uploadOption_plus.previewUploadBtn=previewUploadDiv+"uploadBtn";
            var uploadOpt=uploadOption_plus;
            // 添加初始样式
            var uploadSelect=uploadOpt.uploadSelect.slice(1);
            var selectorDiv='<div id="'+uploadSelect+'" class="hp-upload-div-img"></div>';
          $(uploadOption_plus.previewUploadDiv).append(selectorDiv);
          // 初始化上传组件AjaxUpload
          var au = new AjaxUpload(uploadOpt.uploadSelect, uploadOpt);
          ajaxUpload_plus=au; 
      ...
      viewUploadCss:function(opt){
        var selector="#kdViewUploadSelect"
          ...
          $(uploadOption_plus.previewUploadDiv).append(selectorDiv);
          // 初始化上传组件AjaxUpload
          var au = new AjaxUpload(selector, uploadOpt);
          ajaxUpload_plus=au; 
          //添加对象到元素里
          $(uploadOpt.previewUploadDiv).data("ajaxUpload",au);
      },
  ...
  // 获取对象的方法
  $.fn.extend({
	  getHpUpload:function(){
		  return this.data('ajaxUpload');
      }})
 ...
 *  @id  唯一的guid
 */ 
function removeLocalFiles(localFiles,id){
...
AjaxUpload.prototype = {
  ...
  bindEvent:function(){
    ...
    $(document).on("click",self._settings.previewUploadBtn,function(){
      ...
      var len=self.localFiles.length;
      if(len<=0){
        if(layer){
          layer.msg('请选择图片上传');
        }else{
    ...
    $(document).on("click",self._settings.removeUploadImg,function(){
      if(self._settings.onPreview){
        ...
        if(id){
          removeLocalFiles(self.localFiles,id);
          
        }else{
          console.log('id 没有 请检查代码');
        }
        //移除初始化编辑图
        if(self._settings.isEditInitData){
      	  var idx=$(this).parents('.kdfile').attr('idx');
      	  $('#kd-hidden-img-'+idx).remove();
        }
   ...
   */
    ...
    var jsonData=self._settings.data;
      for(var key in self._settings.data){
        ...
        // 添加文件到formData去
        $.map(self.localFiles,function(item){
          //fd.append('file', item.file);
        	fd.append(self._settings.name, item.file);
        })
  ...
  _createInput : function(){
    ...
    addEvent(input, 'change', function(){
      ...
      if(self._settings.onPreview){
    	 // 变量
    	var kd520=self._settings.uploadSelect.slice(1)+"-kd520"
        // 删除初始样式  重新绑定选择按钮事件
        if($("."+kd520).length<=0){
